...
public class MenuItem extends JPanel {
    ...
    public MenuItem(NewMenu menu, String menus[], int menuIndex, List<MenuEvent> events) {
        this.menu = menu;
        this.menus = menus;
    ...
    private Icon getIcon() {
        ...
        FlatSVGIcon icon = new FlatSVGIcon("trialapp/icons/" + menuIndex + ".svg");
        ...
        for (int i = 0; i < menus.length; i++) {
            ...
            if (i == 0) {
                ...
                menuItem.addActionListener((ActionEvent e) -> {
                    if (menus.length > 1) {
    // Updated condition: Check if the menuShow variable is false
    if (!menuShow) {
        MenuAnimation.animate(MenuItem.this, true);
    } else {
...
if (menuShow) {
    // Adjust arrow size and position based on the menuShow condition
    arrowWidth = UIScale.scale(10);
    arrowHeight = UIScale.scale(5);
    ax = ltr ? (getWidth() - arrowWidth * 2) : arrowWidth;
    ay = (smenuItemHeight - arrowHeight) / 2;
} else {
    arrowWidth = UIScale.scale(4);
    arrowHeight = UIScale.scale(8);
    ax = ltr ? (getWidth() - arrowWidth - UIScale.scale(3)) : UIScale.scale(3);
    ay = (smenuItemHeight - arrowHeight) / 2;
}
